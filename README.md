...
> 对着麦克风说一句话，自动完成文件操作——保存、追加、查看、读取、改名、删除、复制、移动、建文件夹、记待办、整理纪要、文档问答。
...
| 帮我找一下包含「会议」的文件 | `search_files` | 按关键词搜文件名与内容 |
| 统计一下会议记录有多少字 | `count_words` | 返回字数与行数 |
| 把会议记录复制一份叫备份 | `copy_file` | 复制文件到新名字 |
| 把会议记录移到归档文件夹 | `move_file` | 移动文件到子文件夹 |
| 新建一个文件夹叫归档 | `make_dir` | 新建文件夹 |
| 看一下会议记录多大、什么时候改的 | `file_info` | 查看文件大小与修改时间 |
| 记个待办，明天交周报 | `add_todo` | 记一条待办 |
| 看看我的待办 | `list_todo` | 列出待办清单 |
| 买牛奶这件事办好了 | `done_todo` | 标记某条待办完成 |
| 把会议记录整理成纪要 | `summarize_meeting` | 读取会议记录并整理成结构化纪要 |
| 会议记录里关于上线时间怎么说的 | `ask_doc` | 检索 output 下的文档并回答问题（标注来源） |
...
浏览器打开 <http://127.0.0.1:8000>，点「🎙️ 开始录音」说话（首次会请求麦克风权限，说完再点一下停止即自动识别），也可直接输文字指令。
...
- 浏览器内录音用 `MediaRecorder` 录成 webm，本地 whisper 直接解码，音频不出本机；需 Chrome / Edge 等支持 `getUserMedia` 的浏览器，且经 `http://127.0.0.1`（localhost）访问才能使用麦克风
- 首次运行会自动下载 whisper 模型权重并缓存到本地
- 执行结果会用语音播报（edge-tts），形成「说 → 听」闭环；播报前自动清掉 markdown 标记（如 `**加粗**`），避免把符号念出来；页面右上角可一键开关播报
- LLM 与 TTS 调用失败会自动重试（默认共尝试 3 次），重试仍失败时页面会明确提示「语音播报失败」而非静默无反应；重试次数/间隔/超时可在 `.env` 中调整
- 会议纪要整理（说「把会议记录整理成纪要」）会调用 LLM 生成结构化纪要，并另存为「原文件名_纪要.txt」；其余 skill 均为纯本地文件操作
- 本地文档问答（说「会议记录里关于上线时间怎么说的」）是一次轻量 RAG：实时把 output 下的 .txt/.md 切段 → 按中文字符 bigram 检索最相关的 3 段 → 只依据这些片段回答并注明来源文件；不落盘索引，因此文件增删改立刻生效，检索不到会直接说明
