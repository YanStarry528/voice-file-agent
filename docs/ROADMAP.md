...
| 2 | **单元测试** | ✅ 已实现（`tests/` + 根 `conftest.py`） | 覆盖 `safe_output_path`、`_parse_json`、`extract_value` 及各 skill 文件操作，25 个用例 | ★★ |
...
1. **错误兜底优化**（工程细节）——TTS 失败目前静默降级，可补重试与明确提示
2. **Docker / 一键启动**（方向四 #3）——降低他人 clone 后的启动成本
3. **能力横向扩展**（方向三）——依托 `@register` 机制，新增 skill 成本极低
4. **测试深化**——补 `orchestrator` 主流程（mock LLM 响应）、`tool_calling` 循环的测试，提升覆盖率
